...
int main() {
    ...
    const std::string inputPath = "input.csv";
    const std::string outputPath = "output.csv";

    if (GaussianSolver::readSystemFromCSV(inputPath, A, b)) {
        auto start = std::chrono::high_resolution_clock::now();

        if (GaussianSolver::solve(A, b, x)) {
            GaussianSolver::writeSolutionToCSV(outputPath, x);

            auto end = std::chrono::high_resolution_clock::now();
            auto duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);

            std::cout << "System solved in " << duration.count() << " ms\n";
            std::cout << "Solution written to " << outputPath << "\n";
